...
int main() {
    ...
    for (j = 0; j < config.ncat; j++) gdata.permannot[j] = j + 1;  // 1-based to match Fortran
    ...
    for (j = 0; j < gdata.nloci; j++) gdata.vsnptrack[j] = 2;
    for (j = 0; j < gdata.nloci; j++) {
        for (i = 0; i < config.ncat; i++) gdata.snptracker[j][i] = 2;
    }
