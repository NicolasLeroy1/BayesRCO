...
def compare(f1, f2, tol=1e-10):
    ...
    for section in s_f:
        ...
        for i, (val1, val2) in enumerate(zip(v_f, v_c)):
            diff = abs(val1 - val2)
            if diff > tol:
                print(f"MISMATCH in {section}[{i}]: Fortran={val1:.16e}, C={val2:.16e}, diff={diff:.2e}")
                all_match = False
            else:
                print(f"OK {section}[{i}]: F={val1:.10e}, C={val2:.10e}")

    if all_match:
        print("\nSUCCESS: MCMC utils are equivalent between Fortran and C!")
    else:
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python compare_mcmc_utils.py results_f.txt results_c.txt")
        sys.exit(1)
